...
def heatmap_SOM(
        ...
        n_x : int = 4,
        n_y : int = 4,
        som_grid_size : tuple = (10, 10),
        n_iter : int = 100
        ):

        """
        Trains a Self-Organizing Map (SOM) on the dataset, generates heatmaps for each variable, 
        and clusters the data based on the SOM.

        Parameters
        -----------
        data (pd.DataFrame): 
                The dataset containing the variables to be trained on the SOM. The columns represent the variables/features.
        n_x (int):
                The number of subplots along the x-axis of the heatmap grid (default is 4).
        n_y (int):
                The number of subplots along the y-axis of the heatmap grid (default is 4).
        som_grid_size (tuple):
                The size of the SOM grid (default is (10, 10)).
        n_iter (int):
                The number of iterations for training the SOM (default is 100).

        Returns
        --------
        df_bmu : pd.DataFrame
                A DataFrame containing the original data with two additional columns:
                - `BMU`: The Best Matching Unit for each instance in the dataset.
                - `CLUSTER`: The cluster index assigned to each instance based on the BMUs.
        som : MiniSom
                The trained SOM model.
        """

        scaler = StandardScaler()
        data_normalized = scaler.fit_transform(data)

        # Configure the SOM (define the grid size and parameters)
        ...
        # Initialize SOM and train
        som.random_weights_init(data_normalized)
        som.train_random(data_normalized, n_iter)

        # Generate heatmap for each variable
        # For each variable, we check how its values ​​are distributed across the SOM neurons
        plt.figure(figsize=(21, 16))
        for i, col in enumerate(data.columns):
                plt.subplot(n_x, n_y, i + 1)
                weights = som.get_weights()[:, :, i]  # Get the SOM weights for the variable 'i'
        
                plt.pcolor(weights, cmap='coolwarm')  # Generate heatmap for variable 'i'
                plt.title(f"{col}")
        ...
        # Identify the BMUs for each instance
        bmu_indexes = np.array([som.winner(x) for x in data_normalized])
        # bmu_counts = np.zeros(som_grid_size)

        # Count how many times each BMU was a winner
        # for bmu in bmu_indexes:
        #         bmu_counts[bmu] += 1

        df_bmu = data.copy()
        df_bmu['BMU'] = [str(bmu) for bmu in bmu_indexes]

        # Group instances by BMUs and count
        clustered_instances = df_bmu.groupby('BMU').size().reset_index(name='ClusterCount')

        bmu_to_cluster = {bmu: idx for idx, bmu in enumerate(clustered_instances['BMU'])}
        df_bmu['CLUSTER'] = df_bmu['BMU'].map(bmu_to_cluster)
